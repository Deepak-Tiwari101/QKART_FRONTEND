...
const Checkout = () => {
  ...
  const validateRequest = (items, addresses) => {
    const usrBalance = parseInt(localStorage.getItem("balance"));
    const cartBalance = items.reduce((acc, curr) => acc + curr.cost, 0);
    if (usrBalance < cartBalance) {
      enqueueSnackbar(
        "You do not have enough balance in your wallet for this purchase",
        {
          variant: "warning",
  ...
  const performCheckout = async (token, items, addresses) => {
    if (validateRequest(items, addresses)) {
      try {
        ...
        const cartPrice = items.reduce((acc, curr) => acc + curr.cost, 0);

        const newBalance = currBalance - cartPrice;
  ...
  useEffect(() => {
    ...
    getAddresses(token); // fetch addresses if any and set "addresses" state
    onLoadHandler();
